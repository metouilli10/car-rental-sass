...
import { withLocalePath, type AppLocale } from "@/lib/i18n/config";
...
function formatCustomMonthLabel(
  monthValue: string,
  isActive: boolean,
  locale: AppLocale,
  customLabel: string
...
export function PeriodTabs({ period, onPeriodChange, pending = false }: PeriodTabsProps) {
  ...
  const periodItems = [
    { id: "today" as const, label: t("dashboard.periodTabs.today") },
    { id: "7d" as const, label: t("dashboard.periodTabs.sevenDays") },
    { id: "month" as const, label: t("dashboard.periodTabs.thisMonth") },
  ];
  const inputId = useId();
    ...
    <div className="flex flex-nowrap items-center gap-1 overflow-x-auto rounded-lg border border-slate-200 bg-slate-50 p-1 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:inline-flex sm:flex-wrap sm:overflow-visible">
      {periodItems.map((item) =>
        onPeriodChange ? (
          ...
          <Link
            key={item.id}
            href={lp(`/dashboard?period=${item.id}`)}
            className={cn(
              BASE_SEGMENT_CLASS,
              period.key === item.id ? ACTIVE_SEGMENT_CLASS : INACTIVE_SEGMENT_CLASS
            )}
      ...
      <div className="relative">
        ...
        >
          {formatCustomMonthLabel(
            customMonth,
            period.key === "custom",
            locale,
            t("dashboard.periodTabs.custom")
          )}
        </button>
        <input
          id={inputId}
          ...
          aria-label={t("dashboard.periodTabs.monthPickerAria")}
          disabled={pending}
          tabIndex={-1}
