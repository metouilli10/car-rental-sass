    ...
    <Card className="shadow-card">
      ...
      <CardContent className="space-y-0 pt-0">
        <Textarea
          ...
          disabled={isSaving}
        />
        <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-border/70 bg-muted/10 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-sm text-muted-foreground">
            {isDirty
              ? "Modifications non enregistrées"
              : "Les notes sont enregistrées et visibles uniquement par l'équipe."}
          </p>
          <Button onClick={handleSave} disabled={isSaving || !isDirty} className="w-full shrink-0 sm:w-auto">
            {isSaving ? (
              <>
