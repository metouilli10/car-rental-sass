# Locaryx - Copy Instructions (Landing Page)

## Brand Voice
- **Tone:** Professional, reliable, calm urgency, operations-first
- **Promise:** "Votre agence, sous contrôle" (Your agency, under control)
- **Target:** Small car rental agencies in Morocco
- **Language:** French (all UI copy)

---

## HERO SECTION

### Headline
La manière la plus simple de gérer votre agence de location

### Subheadline
Sans erreurs. Sans stress. Sans Excel.

### Description
Locaryx est une application web conçue pour les petites agences de location au Maroc.
Gérez vos véhicules, réservations, clients, contrats et cautions depuis un seul tableau de bord clair et intuitif.

### CTA Buttons
- Primary: → Demander une démo gratuite
- Secondary: Voir comment ça marche ↓

### Trust Badge (Optional)
Déjà adopté par des agences de location à Agadir, Casablanca, Marrakech et Tanger.

---

## PROBLEM → SOLUTION SECTION

### Section Title
Vous reconnaissez ces situations ?

### Problems List
- ❌ Retours en retard oubliés
- ❌ Cautions mal suivies ou perdues
- ❌ Paiements notés sur papier ou WhatsApp
- ❌ Contrats faits à la main
- ❌ Trop de temps perdu à chercher des infos

### Solution Statement
**Locaryx règle tout ça.**

---

## CORE VALUE SECTION

### Section Title
Tout ce dont votre agence a besoin, au même endroit

### Section Subtitle
Locaryx centralise l'opérationnel quotidien de votre agence, sans complexité.

### Features

#### 🚗 Véhicules
Suivez l'état de votre parc en temps réel
Disponible • En location • Maintenance

#### 📅 Réservations
Toutes vos réservations, claires et organisées
Dates, montants, statut, caution

#### 👤 Clients
Historique complet de chaque client
Téléphone, documents, réservations passées

#### 📄 Contrats PDF
Créez des contrats professionnels en quelques clics
Prêts à imprimer ou envoyer

#### 💰 Paiements & cautions
Espèces, virement, carte
Cautions retenues, rendues ou en attente — tout est clair

#### 📊 Rapports
Chiffre d'affaires, taux d'utilisation, performance mensuelle
Prenez de meilleures décisions

---

## DIFFERENTIATOR SECTION (VERY IMPORTANT)

### Section Title
Pensé pour les agences marocaines 🇲🇦

### Section Subtitle
Locaryx n'est pas un logiciel étranger adapté.
Il a été conçu pour votre réalité.

### Key Differentiators
- 💬 WhatsApp intégré (appel & message en 1 clic)
- 💵 Dirhams (MAD), espèces, cautions
- 📄 Contrats adaptés au marché local
- 🧠 Interface simple, même sans expérience informatique
- 🛠 Support humain, en français

---

## SCREENSHOT / ACTION SECTION

### Section Title
Ce que Locaryx surveille pour vous, automatiquement

### Monitoring Features

#### 🔴 Actions requises
Retours en retard, paiements en attente, cautions à gérer.
Locaryx vous montre ce qui peut vous coûter de l'argent aujourd'hui.

#### ⚡ Opérations du jour
Départs et retours à venir, en un coup d'œil.

#### 💼 Caisse du jour
Entrées, sorties et solde — clair, net, sans calcul.

### CTA
(Voir le tableau de bord en démo)
→ Demander une démo gratuite

---

## RESULTS / SOCIAL PROOF SECTION

### Section Title
Des résultats visibles dès les premières semaines

### Key Metrics
- ⏱️ –60% de temps administratif
- 📈 3,5× moins d'erreurs
- 💰 Jusqu'à 95% de cautions mieux suivies

### Testimonial
"Locaryx a changé notre façon de travailler.
On perd moins de temps et on oublie plus rien."
— Gérant d'agence de location

---

## HOW IT WORKS

### Section Title
Démarrer avec Locaryx, c'est simple

### Steps
1️⃣ Demandez une démo
2️⃣ On configure votre agence
3️⃣ Vous commencez à gérer sereinement

### Supporting Text
Aucune installation.
Accessible depuis ordinateur, tablette ou téléphone.

---

## PRICING SECTION

### Section Title
Choisissez le plan adapté à votre agence

### Plans

#### Plan 1: Starter
**Prix:** 799 MAD / mois
**Tagline:** Idéal pour 1 à 5 véhicules

**Features:**
- Gestion des véhicules
- Réservations & clients
- Contrats PDF
- Support inclus

**CTA:** → Commencer maintenant

#### Plan 2: Professionnel (le plus populaire) ⭐
**Prix:** 1499 MAD / mois
**Tagline:** Pour les agences en croissance

**Features:**
- Tout Starter +
- Paiements & cautions
- Tableau de bord avancé
- Rapports détaillés
- WhatsApp intégré

**CTA:** → Demander une démo

#### Plan 3: Entreprise
**Prix:** Sur mesure
**Tagline:** Multi-agences

**Features:**
- Fonctionnalités personnalisées
- Accompagnement dédié

**CTA:** → Nous contacter

### Footer Note
Sans engagement — annulable à tout moment

---

## FAQ SECTION

### Questions & Answers

**Q1:** Est-ce que Locaryx est compliqué à utiliser ?
**A1:** Non. Locaryx est conçu pour être utilisé sans formation technique.

**Q2:** Puis-je l'utiliser sur mon téléphone ?
**A2:** Oui. Locaryx fonctionne sur ordinateur, tablette et mobile.

**Q3:** Mes données sont-elles sécurisées ?
**A3:** Oui. Vos données sont hébergées de manière sécurisée.

**Q4:** Est-ce adapté aux petites agences ?
**A4:** C'est précisément pour elles que Locaryx a été créé.

---

## FINAL CTA

### Headline
Essayez Locaryx gratuitement pendant 14 jours

### Subheadline
Découvrez comment une bonne organisation peut
changer votre quotidien.

### CTA Buttons
- Primary: → Demander une démo gratuite
- Secondary: Créer mon compte

---

## Design Notes

### Colors
- Primary: #6D5EF7 (violet)
- Accent: Use for key CTAs and highlights
- Text: Professional, clean contrast

### Typography
- Headlines: Bold, impactful
- Body: Clear, easy to scan
- CTAs: Action-oriented with arrows (→)

### Imagery
- Dashboard screenshot (hero section)
- Real-world agency contexts
- Moroccan cities/context where appropriate

### Icons
Use emojis strategically for:
- Features (🚗 📅 👤 📄 💰 📊)
- Differentiators (💬 💵 📄 🧠 🛠)
- Monitoring (🔴 ⚡ 💼)
- Metrics (⏱️ 📈 💰)

### Tone in Design
- Calm, controlled, professional
- Avoid overwhelm or chaos
- Emphasize clarity and simplicity
- "Under control" should be felt, not just said
