...
export default async function InspectionsPage({
  ...
  const ins = ui.pageChrome.inspectionsList;
  ...
  const listDescription = interpolate(
    total === 1 ? ins.metaOne : ins.metaOther,
    { count: total }
  );
      ...
      <PageHeader
        ...
        action={
          inspections.length > 0
            ? {
                label: ins.newCta,
                href: withLocalePath(locale, "/damage-reports/new"),
              }
            : undefined
        }
      />
      ...
      {inspections.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center">
            ...
            <p className="text-muted-foreground font-medium mb-2">{ins.emptyTitle}</p>
            <p className="text-sm text-muted-foreground/80 mb-6">{ins.emptyHint}</p>
            <Button asChild>
              <Link href={withLocalePath(locale, "/damage-reports/new")}>{ins.newCta}</Link>
            </Button>
          </CardContent>
        </Card>
      ) : (
                        ...
                        <div className="space-y-1 flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                              ...
                              {inspection.inspectionType === "DEPART" ? ins.typeDepart : ins.typeReturn}
                            </span>
                            <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${
                              hasDamage ? "bg-red-100 text-red-700" : "bg-emerald-100 text-emerald-700"
                            }`}>
                              {hasDamage ? (
                                <><AlertTriangle className="w-3 h-3" /> {ins.damages}</>
                              ) : (
                                <><CheckCircle2 className="w-3 h-3" /> {ins.ok}</>
                              )}
                            </span>
                          </div>
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b bg-gray-50/50">
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thDate}</th>
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thType}</th>
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thClient}</th>
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thVehicle}</th>
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thState}</th>
                      <th className="text-end p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thCost}</th>
                      <th className="text-start p-3 text-xs font-medium text-muted-foreground uppercase tracking-wide">{ins.thDeposit}</th>
                      <th className="p-3"></th>
                    </tr>
                  </thead>
                  <tbody className="divide-y">
                    {inspections.map((inspection) => {
                      const hasDamage = inspection.sections.some((s) => s.status === "DAMAGE");
                      const depositLabel = {
                        RELEASE: ins.depositReleased,
                        PARTIAL: ins.depositPartial,
                        HOLD: ins.depositHeld,
