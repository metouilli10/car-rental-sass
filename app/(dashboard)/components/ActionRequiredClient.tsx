...
// Solid stripe — bg-* classes instead of border-l-* (stripe is a w-0.5 element)
      ...
      <div className="flex items-start justify-between border-b border-gray-100 p-5">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-semibold text-gray-900">Actions prioritaires</h2>
            {items.length > 0 && (
          ...
          <p className="ml-9 mt-1 text-xs text-gray-500">
            {items.length === 0
      ...
      <div className="divide-y divide-gray-50">
        {filteredItems.length === 0 ? (
          ...
          filteredItems.map((item) => (
            ...
            >
              {/* Severity stripe */}
              <div className={cn("h-12 w-0.5 shrink-0 self-stretch rounded-full", stripeClass(item))} />
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-gray-400">{item.dueLabel}</p>
              </div>

              {/* Amount — hidden on mobile */}
              <div className="hidden shrink-0 text-right sm:block">
                <p className="font-semibold text-gray-900">{item.amountText}</p>
                {item.type === "retard" && item.delayDays !== null && item.delayDays > 0 && (
                  <p className="text-xs text-red-500">{item.delayDays}j de retard</p>
                )}
              </div>
